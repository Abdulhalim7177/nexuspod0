    ...
    <div className="space-y-6">
      ...
      {(!pods || pods.length === 0) ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {pods.map((pod: { id: string, title: string, npn: string, summary?: string | null, avatar_url?: string | null, created_at: string, pod_members?: { user_id: string, role: string }[] }) => {
            const myMember = pod.pod_members?.find((m: { user_id: string, role: string }) => m.user_id === user.id)
